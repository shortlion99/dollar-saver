...
const parseExpenseInput = async (inputText) => {
  ...
  for (const expenseText of expensesArray) {
    ...
    const amount = parseFloat(matches[1].replace(/,/g, '')) || parseFloat(matches[2]); 
...
const categorizeExpense = async (description) => {
  const prompt = `Categorize the following entry: "${description}". Is it an expense or an income? Available categories are: ${PREDEFINED_CATEGORIES.join(", ")}. Please respond with the category only from the predefined list and whether it's an expense (true) or income (false).`;
  ...
  try {
    ...
    const validCategory = PREDEFINED_CATEGORIES.includes(category) ? category : "Others";
...
exports.addExpense = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to save expenses. Please try again." });
  }
};
